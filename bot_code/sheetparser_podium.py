...
SHEETID = 893088501 # check URL of sheet for gid=
...
def main(command_name, vehicle_name, current_or_future, image_link):
    ...
    podium_data = sheet_result.get('values', []) # gets the values, or names of vehicles, from the results
    
    if(command_name == 'podiumupdate'):
        ...
        if current_or_future == "Change Current":
            ...
            for i in range(0, len(podium_data) - 1):
                if podium_data[i][2] == 'Current':
                    cur_range = i + range_adj
                elif str(podium_data[i][0]).lower() == str(vehicle_name).lower():
                    del_future_range = i + range_adj
                    is_car_in_future = True
            final_range = 'Podium!A' + str(cur_range) + ':B' + str(cur_range) # only change vehicle name and image link, not cur/future
            values = [
...
#############################################################################################################################################################
            ...
            for i in range(0, len(podium_data) - 1):
                if podium_data[i][2] == 'Future':
                    if str(podium_data[i][0]).lower() == str(vehicle_name).lower():
                    ...
                    "requests": [
                        {
                            "insertDimension": {
                                "range": {
                                    "sheetId": SHEETID,
                                    "dimension": "ROWS",
                                    "startIndex": len(podium_data),
                                    "endIndex": len(podium_data)+1
                                }
                            }
                        }
                    ]
            ...
            service.spreadsheets().values().update(
                spreadsheetId=MY_SPREADSHEET_ID, 
                valueInputOption='USER_ENTERED',
                range='Podium!A' + str(len(podium_data)+1) + ':C' + str(len(podium_data)+1),
                body=body
            ).execute()
